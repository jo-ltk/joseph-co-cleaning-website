...
export default function Footer() {
  ...
  return (
    <footer className="bg-aztec text-yellow-green selection:bg-yellow-green selection:text-aztec">
      ...
      <div className="mx-auto max-w-[1450px] px-5 md:px-10 lg:px-20 py-24">
        <div className="grid gap-16 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="flex flex-col justify-between">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="max-w-[680px]">
              <h2 className="mb-8 text-2xl md:text-4xl leading-[1.1] text-yellow-green">
                Elevating the standards of clean.
              </h2>
              <p className="text-lg leading-relaxed text-yellow-green/70">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.2 }} className="mt-16 flex items-center gap-4">
          ...
          <div className="grid grid-cols-2 gap-12 md:grid-cols-3 lg:gap-16">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.3 }} className="space-y-4">
              ...
              <ul className="space-y-4">
                {pageLinks.map((link) => (
                  <li key={link.href}><Link href={link.href} className="text-base transition-colors duration-300 hover:text-white">{link.label}</Link></li>
                ))}
              </ul>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.4 }} className="space-y-4">
              <span className="block font-mono text-sm uppercase tracking-widest text-yellow-green/40">Social</span>
              <ul className="space-y-4">
                {socialLinks.map((link) => (
                  <li key={link.label}><Link href={link.href} className="text-base transition-colors duration-300 hover:text-white">{link.label}</Link></li>
                ))}
              </ul>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.5 }} className="col-span-2 space-y-4 md:col-span-1">
      ...
      <div className="border-t border-yellow-green/10 px-5 md:px-10 lg:px-20 pt-8 pb-8">
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1, delay: 0.5 }}
          className="mt-8 flex flex-col items-start justify-between gap-4 font-mono text-sm uppercase tracking-wider text-yellow-green/50 md:flex-row md:items-end">
