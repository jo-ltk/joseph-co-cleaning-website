...
function IndustryCard({ title, summary, icon }: { title: string; summary: string; icon: React.ReactNode }) {
  return (
    <article className="group relative min-h-[360px] overflow-hidden bg-aztec/5 transition-colors duration-300 ease-out hover:bg-yellow-green">
      <button type="button" aria-label={`Expand ${title}`} className="absolute right-4 top-4 z-20 flex h-12 w-12 items-center justify-center rounded-full bg-aztec/10 text-2xl font-light leading-none text-aztec transition-colors duration-300 group-hover:bg-aztec/20">
        <span className="-mt-1 transition-transform duration-300 ease-out group-hover:rotate-45">+</span>
      </button>
      <div className="absolute inset-x-0 top-1/2 z-10 flex -translate-y-[40%] flex-col items-center px-8 text-center text-aztec transition-all duration-300 ease-out group-hover:translate-y-[-2rem] group-hover:opacity-0">
        <div className="flex h-[92px] items-center justify-center text-xanadu">{icon}</div>
        <h3 className="mt-6 max-w-[260px] text-xl md:text-2xl leading-[1.1] font-medium tracking-tight text-aztec">{title}</h3>
        <p className="mt-3 text-sm text-aztec/70 max-w-[280px] line-clamp-2">{summary}</p>
      </div>
      <div className="absolute inset-0 z-10 flex items-center px-10 opacity-0 transition-opacity duration-300 ease-out group-hover:opacity-100">
        <div className="max-w-[330px] text-aztec">
          ...
          <p className="mt-8 font-sans text-base leading-relaxed text-aztec/80">{summary}</p>
        </div>
      </div>
    </article>
  );
