...
function IndustryCard({
  ...
  return (
    <article className="group relative min-h-[360px] overflow-hidden bg-[#ececea] transition-colors duration-300 ease-out hover:bg-[#e2ff9d]">
      <button
        type="button"
        aria-label={`Expand ${title}`}
        className="absolute right-5 top-5 z-20 flex h-11 w-11 items-center justify-center rounded-full bg-[#d1d1cf] text-[2rem] font-light leading-none text-[#2d2d2d] transition-colors duration-300 group-hover:bg-[#cfcfcd]"
      >
      ...
      <div className="absolute inset-x-0 top-1/2 z-10 flex -translate-y-[32%] flex-col items-center px-6 text-center text-[#181818] transition-all duration-300 ease-out group-hover:translate-y-[-2rem] group-hover:opacity-0">
        <div className="flex h-[92px] items-center justify-center text-[#666]">
          {icon}
        </div>
        <h3 className="mt-12 max-w-[260px] font-sans text-[1.1rem] font-medium tracking-[-0.05em] text-[#111] sm:text-[1.25rem] md:text-[1.45rem]">
          {title}
        </h3>
      </div>
      ...
      <div className="absolute inset-0 z-10 flex items-center px-10 opacity-0 transition-opacity duration-300 ease-out group-hover:opacity-100">
        <div className="max-w-[330px] text-[#232323]">
          <div className="mb-8 ml-12 h-5 w-5 text-[1.6rem] font-extralight leading-none text-[#cfc8e7]">
            +
          </div>
          <p className="inline-block bg-[#16211b] px-3 py-2 font-sans text-[0.85rem] font-semibold uppercase tracking-[-0.03em] text-[#e2ff9d]">
            Overview:
          </p>
          <p className="mt-8 font-sans text-[1.15rem] leading-[1.3] tracking-[-0.05em] text-[#2a2a2a] md:text-[1.25rem]">
            {summary}
          </p>
        </div>
      </div>
...
export default function IndustriesSection() {
  return (
    <section className="bg-[#f5f5f2] text-[#111]">
      <div className="mx-auto w-full max-w-[1920px] px-5 py-5 md:px-5 md:py-6">
        <h2 className="font-heading text-[2.2rem] font-medium leading-[0.95] tracking-[-0.075em] text-[#111] sm:text-[2.8rem] md:text-[3.2rem]">
          Industries We Serve
        </h2>
