        ...
        <div className="overflow-hidden -mx-5 md:-mx-10 lg:-mx-20">
          <div className="flex w-max gap-8 px-5 md:px-10 lg:px-20 [animation:marqueeScroll_50s_linear_infinite] hover:[animation-play-state:paused]">
            {[...testimonials, ...testimonials].map((item, index) => (
              <motion.article key={`${item.name}-${index}`} className="group relative flex h-[340px] w-[600px] md:h-[380px] md:w-[700px] shrink-0 bg-white rounded-3xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.04)] border border-aztec/5 transition-all duration-500">
                ...
                <div className="flex flex-col justify-between p-10 flex-1">
                  <div>
                    <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-xl bg-yellow-green text-aztec">
                      <Quotes size={20} weight="fill" />
                    </div>
                    <p className="font-sans text-base font-medium leading-relaxed tracking-tight text-aztec">
                      &ldquo;{item.quote}&rdquo;
                    </p>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="h-px w-8 bg-yellow-green" />
                    <div>
                      <h4 className="text-sm font-bold uppercase tracking-widest text-aztec">{item.name}</h4>
                      <p className="text-sm font-medium text-xanadu">{item.location}</p>
                    </div>
                  </div>
                </div>
              </motion.article>
